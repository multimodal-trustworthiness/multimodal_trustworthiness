...
class TradLstm(nn.Module):
    ...
    def __init__(self, batch_size, input_size, hide_size, out_size=1, res_drop=0.0, bi=False, num_layers=1,
        ...
        self.use_cv = use_cv
        if self.bi:
            self.lstm_model = nn.LSTM(input_size, hide_size, num_layers, bidirectional=True, batch_first=True,
                                      dropout=0.1)
        else:
...
def model_train(model, modal, train_loader, valid_loader, test_loader, lr=0.001):
    ...
    epochs = 40
    sample = [f"lstm_{modal}_{time_stamp}.pt"]
    record_loss = {
    ...
    for epoch in range(1, epochs + 1):
        ...
        for idx_batch, (batch_x, batch_y, batch_meta, batch_cv, batch_info) in enumerate(train_loader):
            ...
            if "fusion" not in modal:
                ...
                if idx_batch % 30 == 0:
                    print("Epoch {:2d} | Batch {:2d}/{:2d} | Time {:5.4f} sec | Train Loss {:5.4f}"
                          "".format(epoch, idx_batch, len(train_loader), batch_end - batch_start,
                                    proc_loss / proc_size))
        ...
        if val_loss < best_valid:
            ...
            print(f"Save model at pre_train_models/lstm_{modal}_{time_stamp}.pt!")
            save_model(name=f"lstm_{modal}_{time_stamp}", model=model)
            best_valid = val_loss

    # recording the results of epochs
    log_path = f"{Path.cwd()}\\pre_train_model\\record_val_loss.txt"
    ...
    with open(log_path, "a+") as file:
        ...
        file.close()
        print("-" * 50)
        print(f"Record has been saved in {log_path}")
        print("-" * 50)

    #  test
    model = load_model(f"lstm_{modal}_{time_stamp}")
...
def save_model(name, model):
    ...
    print("-" * 50)
    print(f"model  has been saved in {model_dir}")
    print("-" * 50)
